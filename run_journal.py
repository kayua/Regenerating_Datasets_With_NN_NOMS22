...
DEFAULT_OUTPUT_FILE = "out_journal.txt"
...
def main():
    ...
    cs3 = Campaign(datasets=["S3"], dense_layers=[3], thresholds=[.75], pifs=[None])
    cs4 = Campaign(datasets=["S4"], dense_layers=[3], thresholds=[.75], pifs=[None])
    cs5 = Campaign(datasets=["S5"], dense_layers=[3], thresholds=[.75], pifs=[None])
    cs6 = Campaign(datasets=["S6"], dense_layers=[3], thresholds=[.75], pifs=[None])
    ...
    if args.campaign == "sbrc":
        campaigns = [c1, c2, c3]
    elif args.campaign == "demo":
        campaigns = [cdemo]
    elif args.campaign == "avaliacao":
        campaigns = [ce]
    elif args.campaign == "cs":
        campaigns = [cs6]
                ...
                for dense_layer in c.dense_layers:
                    ...
                    for pif in c.pifs:
                        ...
                        for threshold in c.thresholds:
                            ...
                            check_files([original_swarm_file])
                            ...
                            if pif is None:
                                cmd += "--skip_analyse "
                                cmd += "--failed_swarm_file {} ".format(original_swarm_file) #queremos arrumar o original

                            else:
                                cmd += "--pif {} ".format(pif)
                                cmd += "--original_swarm_file {} ".format(original_swarm_file)
                                cmd += "--failed_swarm_file {} ".format(failed_swarm_file)
                                cmd += "--analyse_file {} ".format(args.output)
